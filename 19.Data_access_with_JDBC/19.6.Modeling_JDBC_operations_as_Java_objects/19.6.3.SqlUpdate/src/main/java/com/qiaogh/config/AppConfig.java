...
public class AppConfig implements AppConst {
    ...
    @Bean
    public SqlUpdate sqlUpdate( final DataSource dataSource ) {
        return new SqlUpdate() {
            @Override
            public void afterPropertiesSet() {
                this.setDataSource( dataSource );
                this.setSql( "update persons set name = ?, age = ? where id = ?" );
                this.declareParameter( new SqlParameter( "name", Types.VARCHAR ) );
                this.declareParameter( new SqlParameter( "age", Types.INTEGER ) );
